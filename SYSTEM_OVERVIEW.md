...
- **Worker incoming-instant-job realtime subscription filters on the wrong id.** `FRONTEND/src/pages/worker/WorkerDashboard.jsx` subscribes to `job_worker_requests` INSERTs with `filter: worker_id=eq.${user.id}` (the Supabase auth user id). But `job_worker_requests.worker_id` is a foreign key to `worker_profiles.id` (`BACKEND/models.py`), a distinct UUID from `users.id` — the component even has `workerProfile` in scope from `useWorkerProfile()` a few lines above but doesn't use its `.id` here. In practice this means the realtime push that's supposed to open `IncomingJobModal` will only ever match by coincidence, so the incoming-instant-job popup is effectively broken via this channel (verified against current code, not a historical claim).
- **A few verified, still-live frontend/backend field-name mismatches** (carried over from an old audit doc, re-checked against the current `JobResponse` schema and are still true today): `FRONTEND/src/pages/job/SearchingPage.jsx` reads `data.assigned_worker` from `GET /jobs/{id}`, but `JobResponse` has no such field (only `worker_id`/`worker_name`), so the matched-worker card never populates on that page. `FRONTEND/src/pages/job/JobDetailPage.jsx` gates its "Open Chat" button on `job.chat_id`, and `FRONTEND/src/pages/admin/AdminJobs.jsx` reads `job.final_amount` for its Amount column — neither field exists on `JobResponse` (the real field is `final_price`, and there is no `chat_id` on a job at all), so the chat button on that page never renders and the admin jobs table's Amount column always shows "—".
- **`BACKEND/BACKEND_SPEC.md`** and the other loose root-level/legacy planning docs (`README.md`, `TODO.md`, `IMPLEMENTATION_AUDIT.md`, `JOB_COMPLETION_FLOW_PLAN.md`, `KAARGAR_BACKEND_UPDATE.md`, `Kaargar_UI_CrossCheck.md`, `FRONTEND/FRONTEND_UI_SPEC.md`, `FRONTEND/FRONTEND_UPDATE.md`) have now been read in full and archived with a `_LEGACY` suffix in the same directory as each original (e.g. `BACKEND/BACKEND_SPEC_LEGACY.md`, `README_LEGACY.md`). All described a mix of the never-built custom-JWT/OTP auth flow, since-changed commission rates (15%/10-15% vs the current 12%/10-15% DB-config-driven model), a two-bucket storage design, and/or work that has since actually been built (the job-completion approval/OTP flow, the support-ticket system, search response wrapping, `WorkerProfileUpdate` field aliases, `JobCreate.budget_max`) — none of that is still-outstanding work. The two genuinely-still-true items they surfaced (the realtime filter bug and the field-name mismatches above) have been folded into this list; everything else in those docs is either already covered elsewhere in this file or confirmed stale. Note: the tools used for this cleanup pass could not rename/delete files directly, so the original stale files at their old paths (e.g. `BACKEND/BACKEND_SPEC.md`, `README.md`) still need to be manually deleted — the `_LEGACY` copies are additive, not replacements, until that's done.
