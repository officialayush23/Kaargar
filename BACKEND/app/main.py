...
async def startup():
    ...
    # Creating a pool with the URL from env
...
# We use HTTPBearer to make Swagger UI work automatically
security = HTTPBearer()
...
async def require_admin(credentials: HTTPAuthorizationCredentials = Depends(security)):
    """
    Checks for Service Role Key OR Admin Role in DB.
    """
    token = credentials.credentials
    
    # 1. Check if it is the Service Role Key directly
    if is_admin_token(token):
        return {"service_role": True}

    # 2. Check if user is admin in DB
    payload = verify_and_decode_jwt(token)
    sub = payload.get("sub")
    ...
    # Assuming role is a text[] array in Postgres
    if row and row["role"] and "admin" in row["role"]:
        return {"service_role": False, "user_id": sub}
    
    raise HTTPException(status_code=403, detail="Admin privileges required")

async def _is_admin_or_owner(target_user_id: str, token: str) -> bool:
    if is_admin_token(token):
        return True
    try:
        payload = verify_and_decode_jwt(token)
...
async def me(payload = Depends(require_user)):
    ...
    if not row:
        # If user exists in Auth but not DB, we might want to return 404 or handle Upsert
        raise HTTPException(status_code=404, detail="User not found in database. Call /upsert_user first.")

    user = dict(row)
    # Sanitize nulls for frontend
    user["professions"] = user.get("professions") or []
    user["skills"] = user.get("skills") or []
    user["tags"] = user.get("tags") or []
...
@app.post("/api/auth/upsert_user")
async def upsert_user_endpoint(payload = Depends(require_user)):
    uid = payload.get("sub")
    email = payload.get("email")
    user_meta = payload.get("user_metadata") or {}
...
async def update_user(user_id: str, payload: UserUpdatePayload, actor = Depends(require_user)):
    ...
    async with app.state.db_pool.acquire() as conn:
        ...
        # Update worker profile if specific fields are present
        if any([payload.professions, payload.skills, payload.hourly_rate_cents is not None, payload.accepts_remote is not None]):
            await conn.execute("""
                INSERT INTO public.worker_profiles (user_id, professions, skills, hourly_rate_cents, accepts_remote, created_at)
                VALUES ($1::uuid, $2::text[], $3::text[], $4::bigint, $5::boolean, now())
                ON CONFLICT (user_id) DO UPDATE
                SET professions = COALESCE($2::text[], public.worker_profiles.professions),
                    skills = COALESCE($3::text[], public.worker_profiles.skills),
                    hourly_rate_cents = COALESCE($4, public.worker_profiles.hourly_rate_cents),
...
async def search_workers(
    ...
    # This relies on your Postgres function `search_workers_for_user_ranked` existing
    async with app.state.db_pool.acquire() as conn:
        rows = await conn.fetch("""
            SELECT * FROM public.search_workers_for_user_ranked(
              NULL::uuid, $1::float8, $2::float8, $3::int, $4::text, $5::bigint, $6::bigint, $7::int
...
async def create_rating(user_id: str, payload: RatingCreate, actor = Depends(require_user)):
    ...
    async with app.state.db_pool.acquire() as conn:
        ...
        # Update Aggregate (Simple approach)
        await conn.execute("""
            UPDATE public.users
            SET rating_count = COALESCE(rating_count, 0) + 1,
                rating_avg = ROUND((COALESCE(rating_avg, 0)::numeric * COALESCE(rating_count, 0) + $1::numeric) / (COALESCE(rating_count, 0) + 1)::numeric, 2)
...
@app.post("/api/admin/toggle_flag")
async def toggle_flag(payload: ToggleFlagPayload, admin = Depends(require_admin)):
    admin_id = admin.get("user_id") if isinstance(admin, dict) else "service_role"
...
@app.get("/api/admin/flagged_users")
async def list_flagged(limit: int = Query(100), offset: int = Query(0), admin = Depends(require_admin)):
    async with app.state.db_pool.acquire() as conn:
        rows = await conn.fetch("""
            SELECT id, name, phone_masked, is_flagged, trouble_score, metadata
            FROM public.users WHERE is_flagged = true
            ORDER BY trouble_score DESC NULLS LAST
            LIMIT $1 OFFSET $2
        """, limit, offset)
