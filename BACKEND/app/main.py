...
async def update_user_profile(payload: UserUpdate, token = Depends(require_user)):
    ...
    if payload.dob:
        age = calculate_age(payload.dob)
        # We need to know current role to enforce 18+, but usually role isn't changing here.
        # Assuming frontend handles basic validation, but good to add check if critical.
...
async def search_workers_endpoint(
    ...
    service: Optional[str] = None, # NEW: Search by service
    gender: Optional[str] = None,
    ...
    """
    Finds workers. 
    Can filter by specific 'service' string (e.g. 'clean').
    """
    async with app.state.db_pool.acquire() as conn:
        rows = await conn.fetch("""
