...
app = FastAPI(title="KAARGAR API 2.2", version="2.2.0")
...
class ProfileCreate(BaseModel):
    ...
    accepts_auto_assign: bool = False # New Field
...
async def upsert_user(payload = Depends(require_user)):
    ...
    async with app.state.db_pool.acquire() as conn:
        ...
        """, uid, email, name)
        # Ensure Governance Stats row exists
...
async def create_profile(payload: ProfileCreate, token = Depends(require_user)):
    ...
    Now includes 'accepts_auto_assign' for workers.
    """
    uid = token.get("sub")
    async with app.state.db_pool.acquire() as conn:
        ...
        if payload.role in ['worker', 'agency', 'company']:
            await conn.execute("""
                ...
                (user_id, worker_type, professions, min_hourly_rate_cents, experience_years, about_text, accepts_auto_assign)
                VALUES ($1, $2::worker_type, $3, $4, $5, $6, $7)
...
async def update_user_profile(payload: UserUpdate, token = Depends(require_user)):
    """ Update Basic Info (For both Workers and Customers) """
    uid = token.get("sub")
    async with app.state.db_pool.acquire() as conn:
        if any([payload.full_name, payload.phone, payload.avatar_url, payload.gender]):
            await conn.execute("""
                ...
                WHERE id = $5
            """, payload.full_name, payload.phone, payload.avatar_url, payload.gender, uid)
    return {"ok": True}

@app.patch("/api/me/worker")
async def update_worker_profile(payload: WorkerUpdate, token = Depends(require_user)):
    """ Update Worker Specifics (Only for Workers) """
    uid = token.get("sub")
    async with app.state.db_pool.acquire() as conn:
        # Check role
...
async def search_workers_endpoint(
    ...
    """
    Finds workers using PostGIS RPC with filters (Price, Gender, etc).
    """
    async with app.state.db_pool.acquire() as conn:
...
async def direct_book_worker(payload: DirectBooking, token = Depends(require_user)):
    """
    PRIMARY FLOW: Check if worker accepts auto-assign.
    If Yes -> 'assigned'. If No -> 'pending_acceptance'.
    """
    customer_id = token.get("sub")
    job = payload.job_details
    
    async with app.state.db_pool.acquire() as conn:
        # Check Worker Preference
        worker = await conn.fetchrow("SELECT accepts_auto_assign FROM public.worker_profiles WHERE user_id=$1", payload.worker_id)
        ...
        # TODO: Trigger Notification to Worker
...
async def rate_worker(payload: RatingCreate, token = Depends(require_user)):
    ...
    async with app.state.db_pool.acquire() as conn:
        ...
        await conn.execute("""
            INSERT INTO public.worker_ratings (job_id, reviewer_id, target_id, rating, comment)
            VALUES ($1, $2, $3, $4, $5)
        """, payload.job_id, uid, payload.target_id, payload.rating, payload.comment)
    return {"ok": True}
...
async def rate_user(payload: RatingCreate, token = Depends(require_user)):
    ...
    async with app.state.db_pool.acquire() as conn:
        ...
        await conn.execute("""
            INSERT INTO public.user_ratings (job_id, reviewer_id, target_id, rating, comment)
            VALUES ($1, $2, $3, $4, $5)
        """, payload.job_id, uid, payload.target_id, payload.rating, payload.comment)
    return {"ok": True}
