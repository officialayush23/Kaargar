...
# --- STATIC ROUTES (Place these BEFORE dynamic {id} routes) ---
...
async def hire_worker(
    ...
    if not data: raise HTTPException(404, "Job or Bid not found, or unauthorized")
    if data["status"] not in ("open", "bidding"): raise HTTPException(400, "Job is not open for hiring")
    if data["balance_cents"] < data["amount_cents"]: raise HTTPException(402, "Insufficient wallet balance")
...
async def book_job(
    ...
    try:
        ...
        if worker["role"] != "worker" or worker["kyc_status"] != "verified": raise HTTPException(400, "Worker unavailable")

        loc_expr = "NULL"
        params = [customer_id] # $1
        idx = 2
        # ... Constructing 13 basic params ...
        base_params = [jd.title, jd.description, jd.category, jd.profession_required, jd.services_required or [], jd.is_remote, jd.budget_min_cents, jd.budget_max_cents, jd.price_type, jd.address_text, jd.city, jd.pincode]
        params.extend(base_params)

        if not jd.is_remote and jd.lat is not None and jd.lon is not None:
            loc_expr = f"ST_SetSRID(ST_MakePoint(${len(params)+2}, ${len(params)+1})::geography, 4326)"
            params.append(jd.lat)
            params.append(jd.lon)
        ...
        if r:
            background_tasks.add_task(notify_bg, r, payload.worker_id, msg_type, {"job_id": str(job_id), "message": msg})

        return {"ok": True, "job_id": str(job_id), "type": "direct" if is_direct else "request"}

    except HTTPException as he: raise he
    except Exception as e:
        logger.error(f"Book Job Error: {e}")
        raise HTTPException(500, str(e))

# --- ME ENDPOINTS ---

@router.get("/api/me/jobs/worked")
async def get_worker_jobs(user: dict = Depends(require_worker), conn: asyncpg.Connection = Depends(get_db)):
    rows = await conn.fetch("""
...
async def get_customer_jobs(user: dict = Depends(require_db_user), conn: asyncpg.Connection = Depends(get_db)):
    rows = await conn.fetch("""
        SELECT j.id, j.title, j.status, j.created_at, j.budget_max_cents, j.budget_min_cents,
               u.full_name as worker_name,
               (SELECT count(*) FROM public.bids b WHERE b.job_id = j.id) as bid_count
        FROM public.jobs j
...
async def get_job(job_id: UUID, user: dict = Depends(require_db_user), conn: asyncpg.Connection = Depends(get_db)):
    row = await conn.fetchrow("""
        SELECT j.*, u_cust.full_name AS customer_name, u_work.full_name AS worker_name,
               jp.worker_proof_imgs, jp.worker_comment, jp.bill_details, jp.worker_submitted_at, jp.customer_approved
        FROM public.jobs j 
...
async def update_job_status(job_id: UUID, payload: JobStatusUpdate, background_tasks: BackgroundTasks, user: dict = Depends(require_db_user), conn: asyncpg.Connection = Depends(get_db), r: Optional[redis.Redis] = Depends(get_redis)):
    uid = user["id"]
    ...
    row = await conn.fetchrow("UPDATE public.jobs SET status = $1::public.job_status WHERE id = $2 AND (customer_id = $3 OR worker_id = $3 OR requested_worker_id = $3) RETURNING *", payload.status, job_id, uid)
    if not row: raise HTTPException(403, "Update failed")
    if r:
        recipient_id = current_job["worker_id"] or current_job["requested_worker_id"] if uid == current_job["customer_id"] else current_job["customer_id"]
        if recipient_id: background_tasks.add_task(notify_bg, r, recipient_id, "job_status_update", {"job_id": str(job_id), "message": f"Job '{current_job['title']}' status: {payload.status}", "new_status": payload.status})
...
async def place_bid(job_id: UUID, payload: BidCreate, background_tasks: BackgroundTasks, worker: dict = Depends(require_worker), conn: asyncpg.Connection = Depends(get_db), r: Optional[redis.Redis] = Depends(get_redis)):
    worker_id = worker["id"]
    ...
    if r:
        ...
        background_tasks.add_task(notify_bg, r, job["customer_id"], "bid", {"job_id": str(job_id), "message": msg})

    return {"ok": True, "bid_id": str(bid["id"]), "auto_accepted": auto_accept}

@router.post("/api/jobs/{job_id}/submit_work")
async def submit_work(job_id: UUID, payload: JobProofSubmit, background_tasks: BackgroundTasks, worker: dict = Depends(require_worker), conn: asyncpg.Connection = Depends(get_db), r: Optional[redis.Redis] = Depends(get_redis)):
    ...
    async with conn.transaction():
        await conn.execute("INSERT INTO public.job_proofs (job_id, worker_proof_imgs, worker_comment, bill_details, worker_submitted_at) VALUES ($1, $2, $3, $4::jsonb, now()) ON CONFLICT (job_id) DO UPDATE SET worker_proof_imgs = EXCLUDED.worker_proof_imgs, worker_comment = EXCLUDED.worker_comment, bill_details = EXCLUDED.bill_details, worker_submitted_at = now()", job_id, payload.worker_proof_imgs, payload.worker_comment, json.dumps(payload.bill_details))
        await conn.execute("UPDATE public.jobs SET status = 'pending_acceptance' WHERE id = $1", job_id)
    if r: background_tasks.add_task(notify_bg, r, job["customer_id"], "work_submitted", {"job_id": str(job_id), "message": f"Work submitted for {job['title']}"})
    return {"ok": True}

@router.post("/api/jobs/{job_id}/approve_work")
async def approve_work(job_id: UUID, payload: JobProofApprove, background_tasks: BackgroundTasks, user: dict = Depends(require_db_user), conn: asyncpg.Connection = Depends(get_db), r: Optional[redis.Redis] = Depends(get_redis)):
    ...
    if r: background_tasks.add_task(notify_bg, r, job["worker_id"], "payment_received", {"job_id": str(job_id), "message": f"Payment released for {job['title']}"})
    return {"ok": True}