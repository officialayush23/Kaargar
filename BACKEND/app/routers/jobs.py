...
async def update_job_status(
    ...
    if r:
        ...
        if recipient_id:
            background_tasks.add_task(notify_bg, r, recipient_id, "job_status_update", {"job_id": str(job_id), "message": msg, "new_status": payload.status})
...
async def hire_worker(
    ...
    if r:
        background_tasks.add_task(notify_bg, r, data["worker_id"], "hired", {"job_id": str(payload.job_id), "message": f"Hired for {data['title']}"})
...
async def submit_work(
    ...
    if r:
        background_tasks.add_task(notify_bg, r, job["customer_id"], "work_submitted", {"job_id": str(job_id), "message": f"Work submitted for {job['title']}"})
...
async def approve_work(
    ...
    if r:
        background_tasks.add_task(notify_bg, r, job["worker_id"], "payment_received", {"job_id": str(job_id), "message": f"Payment released for {job['title']}"})
